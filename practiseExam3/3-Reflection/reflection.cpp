...
bool Reflection::paintGL()
{
    ...
    // Pass 1b: draw scene reflected onto Z=min.z 
	// Uses default shaders
	g.glClearColor(0.8,0.8,0.8,0);
	g.glClear(GL_DEPTH_BUFFER_BIT | GL_COLOR_BUFFER_BIT);
    ...
    // Pass 1c: draw scene reflected onto X=min.x 
	// Uses default shaders
	g.glClearColor(0.8,0.8,0.8,0);
	g.glClear(GL_DEPTH_BUFFER_BIT | GL_COLOR_BUFFER_BIT);
    ...
    // Pass 3b: Draw mirror (a textured quad)
	// Uses our own texture mapping shader
	b = scene()->boundingBox();
    program->bind();
    program->setUniformValue("colorMap", 0);
    ...
    // Pass 3c: Draw mirror (a textured quad)
	// Uses our own texture mapping shader
	b = scene()->boundingBox();
    program->bind();
    program->setUniformValue("colorMap", 0);
